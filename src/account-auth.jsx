...
async function requestPasswordReset(e){
  ...
  try{
   const{error:e2}=await supabase.auth.resetPasswordForEmail(email,{redirectTo:'https://www.ijlanga.co.za/account-verification.html?type=recovery'});
   if(e2)throw e2;
 ...
 if(mode==='signin')return <div className={shell}><div className="account-auth-card">{onClose&&<button className="account-close" onClick={onClose} aria-label="Close"><X/></button>}<button className="auth-back" onClick={()=>go('choice')}><ArrowLeft size={16}/> Back</button><span className="account-eyebrow">SIGN IN</span><h1>Welcome back.</h1><p>Enter your account details. Your profile determines whether you are sent to Admin, Employer, Employee, Reseller or Client.</p><form className="auth-form" onSubmit={signIn}><label>Email Address<input type="email" value={form.email} onChange={e=>update('email',e.target.value)} required autoComplete="email"/></label><label>Password<div className="password-field"><input type={showPassword?'text':'password'} value={form.password} onChange={e=>update('password',e.target.value)} required autoComplete="current-password"/><button type="button" onClick={()=>setShowPassword(!showPassword)} aria-label={showPassword?'Hide password':'Show password'}>{showPassword?<EyeOff size={17}/>:<Eye size={17}/>}</button></div></label>{error&&<div className="auth-error">{error}</div>}{notice&&<div className="auth-notice">{notice}</div>}<button className="auth-primary" disabled={busy}>{busy?'Signing in…':'Sign in'}</button></form><div className="auth-link-row"><button className="auth-link-button" disabled={busy} onClick={()=>go('reset-request')}>Forgot password?</button><button className="auth-link-button" disabled={busy} onClick={()=>go('magic-link')}>Sign in with email link</button><button className="auth-link-button" disabled={busy} onClick={resendConfirmation}>Resend confirmation</button><a className="auth-link-button" href="/account-verification.html">Verify account</a></div><div className="auth-switch">Need an account? <button onClick={()=>go('signup')}>Sign up</button></div></div></div>;

 if(mode==='magic-link')return <div className={shell}><div className="account-auth-card">{onClose&&<button className="account-close" onClick={onClose} aria-label="Close"><X/></button>}<button className="auth-back" onClick={()=>go('signin')}><ArrowLeft size={16}/> Back to sign in</button><span className="account-eyebrow">MAGIC LINK</span><h1>Sign in without a password.</h1><p>Enter your email address and we will send a one-time secure sign-in link.</p><form className="auth-form" onSubmit={sendMagicLink}><label>Email Address<input type="email" value={form.email} onChange={e=>update('email',e.target.value)} required autoComplete="email"/></label>{error&&<div className="auth-error">{error}</div>}{notice&&<div className="auth-notice">{notice}</div>}<button className="auth-primary" disabled={busy}>{busy?'Sending…':'Send sign-in link'}</button></form></div></div>;

if(mode==='reset-request')return <div className={shell}><div className="account-auth-card">{onClose&&<button className="account-close" onClick={onClose} aria-label="Close"><X/></button>}<button className="auth-back" onClick={()=>go('signin')}><ArrowLeft size={16}/> Back to sign in</button><span className="account-eyebrow">PASSWORD RESET</span><h1>Reset your password.</h1><p>Enter the email address connected to your IJ Langa account. We will send a secure reset link to that address.</p><form className="auth-form" onSubmit={requestPasswordReset}><label>Email Address<input type="email" value={form.email} onChange={e=>update('email',e.target.value)} required autoComplete="email"/></label>{error&&<div className="auth-error">{error}</div>}{notice&&<div className="auth-notice">{notice}</div>}<button className="auth-primary" disabled={busy}>{busy?'Sending reset email…':'Send password reset email'}</button></form><small className="account-note">For security, we do not reveal whether an email address is registered.</small></div></div>;
