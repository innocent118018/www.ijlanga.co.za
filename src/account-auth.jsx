...
export default function AccountAuth({onClose=false,embedded=false}){
 ...
 const[form,setForm]=useState({email:'',password:'',first_name:'',last_name:'',surname:'',phone:'',identity_number:''});
 ...
 async function resetPassword(){
  ...
  try{
   const{error:e}=await supabase.auth.resetPasswordForEmail(form.email.trim().toLowerCase(),{redirectTo:window.location.origin+'/dashboard.html'});
   if(e)throw e;
   setNotice('If an account exists for this email address, a password-reset email has been sent.');
 ...
 async function signUp(e){
  ...
  try{
   ...
   Object.entries(form).forEach(([k,v])=>{if(k!=='password')fd.append(k,v)});
   ...
   fd.append('id_copy',idCopy);
   fd.append('proof_of_address',proofOfAddress);
   ...
   setNotice(data.message||'Registration submitted for approval.');
   setMode('submitted');
