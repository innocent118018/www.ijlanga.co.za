 ...
 useEffect(()=>{
   ...
   const run=async()=>{
    try{
      ...
      if(tokenHash){
        const verifyType=type==='signup'||type==='confirmation'?'email':type;
        let verification=await supabase.auth.verifyOtp({token_hash:tokenHash,type:verifyType});
        // Older registration links may carry a magiclink label while their
        // token was generated by the signup flow. Retry with the signup/email
        // verification type before reporting a failure.
        if(verification.error && verifyType==='magiclink'){
          verification=await supabase.auth.verifyOtp({token_hash:tokenHash,type:'email'});
        }
