...
function customConfirmUrl(action: string, hash: string, redirectTo: string) {
  ...
  return "https://www.ijlanga.co.za/auth-confirm.html?" + qs.toString();
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json({error:{http_code:status,message:error?.message || "Authentication email hook failed."}},status);
  }
});